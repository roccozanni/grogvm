...
describe('stepAllActorWalks', () => {
  ...
  const stubCostume = {
    header: {
      ...
      animOffsets: new Array(32).fill(0),
      mirrorFlag: false,
    },
  ...
  it('drives the walk chore (walkFrame*4 + dir) while moving', () => {
    ...
    stepAllActorWalks(vm);
    expect(a.facing).toBe('E');
    // walkFrame 2 * 4 + dir E(1) = 9.
    expect(a.anim.animId).toBe(9);
  });

  it('settles to the directional init pose on arrival', () => {
    const vm = vmWithCostume();
    const a = vm.actors.get(1);
    a.costume = 1; a.x = 0; a.y = 0; a.walkTarget = { x: 0, y: -1 }; a.isMoving = true;
    stepAllActorWalks(vm); // steps north and arrives this tick (|dy|=1 ≤ speedY)
    expect(a.isMoving).toBe(false);
    expect(a.facing).toBe('N');
    // initFrame 1 * 4 + dir N(3) = 7.
    expect(a.anim.animId).toBe(7);
  });

  it('does not touch an idle actor\'s anim (no clobbering script-driven FX)', () => {
    const vm = vmWithCostume();
    const a = vm.actors.get(1);
    a.costume = 1; a.x = 10; a.y = 10; // not moving
    a.anim = { ...a.anim, animId: 2 }; // a script-driven FX anim (e.g. a sparkle)
    stepAllActorWalks(vm);
    expect(a.anim.animId).toBe(2); // untouched
  });
});
