...
Engine fixes made along this arc (kept here until folded into `pages/docs/`):

- **`wait forCamera` deadlock fix (2026-06-10, confirmed in-browser).** The trio conversation
  (#220, SCUMM bar room 28) froze before its first line: room 28's camera script #201 re-issues
  `panCameraTo` to the camera's CURRENT position every frame, and our "armed-pan" wait check
  deadlocked every later-slot waiter that ever yielded there (one-way, scheduling-dependent —
  why all headless probes and the walkthrough passed while the browser run lost the race). Fix:
  forCamera waits until the camera *reaches* its destination. Unit-pinned (wait.test.ts);
  diagnosed from the user's broken-state save (probe: `scratch/broken-save-probe.ts`).
  Written up in [engine/camera.md §3](pages/docs/engine/camera.md).
- **Narrator-text (print a=255) lifetime fix (2026-06-10, confirmed in-browser).** A close-up
  conversation's last reply lingered over the dialog choices: we routed every out-of-range print
  actor to `systemTexts`, which never timer-clears. The game data splits the channels — a=255 is
  talk-flow (erased at drain unless keepText `\xff\x02`; every persistent a=255 in MI1 carries
  it), a=254 is the blast channel (dance steps, room-36 disclaimer; restoreCharsetBg lifetime).
  Fix: the drain sweeps non-keepText a=255 entries. Unit-pinned (talk.test.ts + vm.test.ts).
  Written up in [scumm/char.md](pages/docs/scumm/char.md) ("The message channels" + "The talk
  timer vs. on-screen lifetime" — both sections corrected).
...
- (none — the forest-maze occlusion saves are resolved; see the closed **H**
  item below. Dev save at the forest fork: `saves/MI1-forest-fork.websave.json`,
  gitignored, regenerated by `scratch/forest-fork-save.ts`. The trio-deadlock
  save `saves/MI1-Italiano-quicksave.websave.json` is resolved by the
  `wait forCamera` fix above; probe: `scratch/broken-save-probe.ts`.)

### Open issues (reported, not yet investigated)

- (none. The VM-inspector staleness reported 2026-06-10 is fixed, confirmed
  in-browser: the drawer rebuilt its heavy live tables ONLY on non-playing
  frames — and the drawer's own Pause button stops the clock, so no frame
  followed and not even pausing refreshed them. Now: throttled rebuild during
  play (`LIVE_REPAINT_MS` = 250 ms; per-frame rebuilds jank camera-follow),
  full-rate when paused/stepping, plus an immediate rebuild on the Pause
  click. `debug.ts`.)
...
- [x] **H — forest maze z-plane occlusion: FIXED (2026-06-10, confirmed
  in-browser).** Root cause: SCUMM's mask planes are a stateful surface
  written in draw order, per 8-px strip (opaque strip replaces — zeros where
  the object has no data — transparent strip ORs); our order-blind OR let the
  parked-under exit masks survive and eat ego. Same rule makes the #673
  orphan harmless (no auto-draw gating needed) and explains/retires the
  all-1s-drop heuristic. Unit-pinned (compositor draw-order suite) and
...
- [ ] **M — system `print` text lifetime: `restoreCharsetBg` approximated by
  ...
  redraws** (`vm.ts advanceOrEndTalk` + `endCutscene` + `moveCameraTo`).
  *Narrowed 2026-06-10:* the narrator channel (a=255) now drains faithfully
  with the talk timer (see the lifetime fix in Current above), so this item
  is only about **a=254** blast text and the trigger approximations below. Faithful
...
- **Unify the render surface — DONE (Phase B shipped 2026-06-10; see the
  `## DONE — Frame ownership` record below).** Phase A (one screen canvas +
  unified input coords) shipped earlier; Phase B moved text/verb composition
  into the engine so it emits the whole 320×200 framebuffer.
...
- **Audio timing — DONE (2026-06-09, this session).** The `AudioBackend` seam
  + `SilentTimingBackend` ship: `isSoundRunning` now reports truthfully for each
  sound's real duration, so sound-gated cutscenes/transitions hold instead of
  collapsing. Durations are parsed from the resources (SBL VOC time-constant,
  MIDI tempo×ticks) and, for the CD-trigger sounds, read live from the
  `TrackN.fla` FLAC headers at load time (partial, header-only). Writeups:
  the seam → [engine/audio.md](pages/docs/engine/audio.md); the SOUN formats +
  gate idiom → [scumm/sound.md](pages/docs/scumm/sound.md). Actual audio
  **OUTPUT** (`WebAudioBackend`) remains a separate later phase behind the
  same interface.
...
- **Frame ownership** (REVIEW.md item 2) — DONE 2026-06-10 (see
  `## DONE — Frame ownership` below).
...
## DONE — Frame ownership (REVIEW.md item 2)

Implemented + confirmed in-browser 2026-06-10. The engine now emits the
COMPLETE visible screen — room slice + verb/inventory panel + dialog/system
text — as one indexed framebuffer through the Renderer seam; the shell paints
only the cursor crosshair and debug overlays over the blit. Writeups:
[engine/architecture.md](pages/docs/engine/architecture.md) "Frame ownership"
and [engine/session.md](pages/docs/engine/session.md) §4/§8. This is the
closure record.

What shipped (`src/engine/render/screen.ts` + `screen.test.ts` new;
session/types/play-area/play/input restructured; vm gained `getCharset` /
`getRoom` cached accessors mirroring `getCostume`; boot's charset resolver
folds in the walk-order fallback for builtin ids 0/5):

- **`composeScreen`** assembles the screen around the camera-sliced,
  post-shake room band: verb band (text verbs in their own charsets, image
  verbs blitted indexed with source-room TRNS, hover/armed ink), dialog +
  system text clipped to the room band. All relocated verbatim from
  play-area.ts — per-line centring, hiColor-0 no-flash, overhead anchoring,
  edge clamps, live talkColor. **`verbAt`** (same module) is the one verb
  hit-test, shared by the compose-time hover highlight and the shell's click
  routing.
- **Session**: `FrameInfo.width/height` are screen dims (typically 320×200)
  with `viewportWidth`/`roomHeight` alongside for the shell's input band
  split; new `session.present()` (compose without ticking) keeps paused
  hover live. The armed-verb global (#107 — an MI1 script convention, noted
  in session.ts) and mouse VARs 44/45 feed the highlight.
- **Forced behavior changes** (both deliberate, verified): no-room frames
  present with the last-seen palette (baked-in text must stay visible);
  the frame presents OPAQUE (`setTransparentIndex(null)` — the old
  alpha-holes were an artifact of presenting a partial frame).
  **Deliberate drop:** the `(no charset loaded)` browser-font fallback —
  degradation is now band fill + image verbs, text verbs skipped.
- **Caveat:** headless writers (`sendInput`, testkit `setMouse`) put ROOM
  coords in VARs 44/45, so headless hover can't land in the verb band —
  ink-only, pre-existing; tests wanting band hover write script-screen
  coords.
- Net: play-area.ts shrank ~770→~300 lines (caches, text rasterizer, verb
  painter, duplicate hit-test all deleted); MemoryRenderer integration tests
  now assert verb-text pixels through the seam; the screen composer has its
  own unit suite (band assembly, ink selection, clipping, dialog placement,
  hit-test precedence).
...
Implemented 2026-06-09 (this session) per the plan that lived here. The
writeups now live in the docs — the seam in
[engine/audio.md](pages/docs/engine/audio.md), the SOUN formats + gate idiom
in [scumm/sound.md](pages/docs/scumm/sound.md); this is just the closure record.

What shipped (`src/engine/sound/` + wiring in `vm.ts`/`opcodes/index.ts`/
`boot.ts`/`scripts.ts`/`savestate.ts`; CD-track header reads in
`testkit/scummv5.ts` (node) + `platform/storage/game-files.ts` (web)):
- `AudioBackend` seam + the one `SilentTimingBackend` (timing only, no output),
  wired through `bootGame` so **every** session including web play uses it.
- `parseSound` → `SoundResource{durationJiffies, looping}` from the real SOUN
  data: SBL via its embedded **VOC time-constant** (rate is per-sound, not a
  fixed guess — MI1 decodes to ~6849 Hz), MIDI via tempo×ticks.
- `isSoundRunning` now reads the backend; the backend drains a jiffy per
  `beginTick`. Savestate delegates to `serialize`/`restore` (SAVE_VERSION → 2).
- Unit-pinned (synthetic + real-MI1 durations) and the from-boot integration net
  is green + deterministic.

STEP-0 finding that **corrected the plan's premise**: the plan assumed the
wait-gated sounds are SBL/MIDI; in fact **5 of 7 are CD-track triggers**
(24-byte `0x18…` commands; byte 16 = CD track, byte 17 = `01` one-shot / `ff`
loop). Their durations are the redbook track lengths, which live in the
`TrackN.fla` files (real FLAC), not `MONKEY.001`. So the boot caller reads each
track's FLAC **STREAMINFO** up front — a partial, header-only read (~42 bytes,
never the multi-MB body) — and hands the VM a plain `track → jiffies` map
(`cdTrackDurations`), same shape as the other resources it loads eagerly: node
via `fs` (`bootScummV5` → `readCdTrackDurations`), browser via `File.slice`
(`loadSessionGame`). All gated CD tracks are one-shot, so none can hang; looping
CD music never gates.

Regression-net side effect (expected, **more** faithful): sound-gated holds now
advance game-frames — and with them the ambient RNG stream — so the seeded
insult-duel grind converges in ~38 duels instead of 30. The grind beat's CAP was
a backstop, not the exact count; bumped to 60 with the cause noted in the test.

**Follow-ups (in-browser feedback, 2026-06-09):**
- **ESC-skip fix for "le tre prove" (g#57).** With the gate now actually holding,
  ESC didn't skip it. Root cause: `g#57` ends its setup cutscenes, *then* arms
  `beginOverride` for the trials intro — so the gate runs at the cutscene-stack
...
- **English CD release (MP3 tracks).** `games/MI1-EN-CD-DOS-VGA` ships the music
  ...
  magic); the track readers now match `Track*.{fla,mp3}`. EN durations match the
  IT FLAC to within ~2 jiffies. Unit + EN-data-gated tests pin it.
- **MI2 sanity (`games/MI2`, no external tracks).** Boots and runs 3000+ ticks
  with no halt; 199 SOUN blocks all `SOU ` containers (no CD triggers) and all
  parse cleanly — the seam handles MI2 for free via the SBL/MIDI path, CD path
  unused. (Full MI2 support still its own later task.)
