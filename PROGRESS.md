...
**FIXED 2026-06-14 — LeChuck-finale "punched before I can grab the root beer"**
(in-browser report, save `saves/MI1-Italiano-quicksave.websave.json`). Room 59:
the stand-up cutscene (#129) arms TWO punch triggers — **#126** (LeChuck windup +
3×`delay 180` ≈ the ~9 s "do nothing" timer) and **#125** (`while ego is NOT
moving: loop; the instant it moves → doSentence STOP + punch`, the "don't try to
move" gate). The root beer (#733) sits on ego's OWN walk-spot (297,106 / face W,
== ego after the stand-up), so grabbing it should need no walk and no turn → not
trip #125. **Root cause:** `startWalk` flagged `isMoving=true` for one frame even
on a zero-distance `walkActorToObject`, and scripts run BEFORE the walk step each
frame, so #125 read the phantom "moving" and pre-empted the pickup. Fix =
SCUMM `startWalkActor` early-out (already-at-target ⇒ no movement). That 1-frame
shift then exposed a latent deadlock: the general-store door exit runs the
open-door verb's `cutScene…endCutScene` (#387) while its sibling walk-to-door
drives `loadRoomWithEgo`; the room change killed the still-animating cutscene
before its `endCutScene` → stranded frame → frozen controls. Fix =
`stopRoomLocalScripts` now spares ANY slot that owns an active cutscene frame
(generalises the carried-object spare; mirrors the `freezeScripts` cutscene-caller
spare). Guard: `walk.test.ts` zero-distance early-out + the integration door beat
(which goes red without the cutscene-caller spare). Finale verified end-to-end
from the save: stand up → grab root beer (no punch) → use on LeChuck → win (#132).
